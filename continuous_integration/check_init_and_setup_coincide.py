...
def main() -> int:
    ...
    if setup_py_map["version"] != bodygamepad.__version__:
        print(
            f"The version in the setup.py is {setup_py_map['version']}, "
            f"while the version in bodygamepad/__init__.py is: "
            f"{bodygamepad.__version__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["author"] != bodygamepad.__author__:
        print(
            f"The author in the setup.py is {setup_py_map['author']}, "
            f"while the author in bodygamepad/__init__.py is: "
            f"{bodygamepad.__author__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["license"] != bodygamepad.__license__:
        print(
            f"The license in the setup.py is {setup_py_map['license']}, "
            f"while the license in bodygamepad/__init__.py is: "
            f"{bodygamepad.__license__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["description"] != bodygamepad.__doc__:
        print(
            f"The description in the setup.py is {setup_py_map['description']}, "
            f"while the description in bodygamepad/__init__.py is: "
            f"{bodygamepad.__doc__}",
            file=sys.stderr,
        )
        success = False
    ...
    if status_classifier is None:
        ...
        if expected_status_in_init != bodygamepad.__status__:
            print(
                f"Expected status {expected_status_in_init} "
                f"according to setup.py in bodygamepad/__init__.py, "
                f"but found: {bodygamepad.__status__}"
            )
            success = False
