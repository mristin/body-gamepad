...
def main() -> int:
    ...
    if Step.DOCTEST in selects and Step.DOCTEST not in skips:
        ...
        for pth in (repo_root / "bodygamepad").glob("**/*.py"):
    ...
    if (
        ...
        print("Checking that bodygamepad/__init__.py and setup.py coincide...")
        exit_code = call_and_report(
            verb="check that bodygamepad/__init__.py and setup.py coincide",
            cmd=[
                sys.executable,
        ...
        print("Skipped checking that bodygamepad/__init__.py and setup.py coincide.")

    # NOTE (mristin, 2023-04-15):
    # We need to check for the Python version since ``argparse`` output changes
    # between the versions. Hence, we pin it at the moment to Python 3.9.
